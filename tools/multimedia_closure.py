...
requirement, which factory recipe answers it. This module is the check that the
inventory is true of the tree: a requirement with no entry, an entry naming a
recipe that does not exist or is not eligible to build, and an entry for
something the transaction does not ask for are all contract violations.
