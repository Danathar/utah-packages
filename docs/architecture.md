...
The root Packit configuration and the source lock both cover all 342 recipes:

| check | result |
| --- | ---: |
| `ls -d packages/*/ \| wc -l` | `342` |
| entries under `.packit.yaml:packages` | `342` |
| entries under `config/upstream-sources.json:packages` | `342` |

`python3 tools/validate.py` reports:

```text
validated 342 source RPMs
```
...
| **Stage assignment** | 65 of 342 packages carry a hand-assigned `stage` | Solve waves from real BuildRequires; config `stage` demotes to an override for cycle-breakers such as `malcontent-bootstrap` | `preflight` already resolves every recipe's BuildRequires and then discards the result. Hand integers are a manual cache of a computed value; two of them were discovered by a build failing. Hummingbird has no stage numbers at all — it is solver-driven plus a reverse-dependency impact scanner. |
