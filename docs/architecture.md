...
The root Packit configuration and the source lock both cover all 349 recipes:

| check | result |
| --- | ---: |
| `ls -d packages/*/ \| wc -l` | `349` |
| entries under `.packit.yaml:packages` | `349` |
| entries under `config/upstream-sources.json:packages` | `349` |

`python3 tools/validate.py` reports:

```text
validated 349 source RPMs
```
